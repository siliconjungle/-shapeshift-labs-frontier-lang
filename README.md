...
console.log(compileFrontierSource(source, { target: "javascript" }).output);
```
...
- Language adapters depend on kernel.
- Compiler depends on parser, checker, and projection adapters.
- CLI depends on the compiler and owns Node filesystem/bin behavior.
- Umbrella depends on the package family and re-exports it for convenience.
